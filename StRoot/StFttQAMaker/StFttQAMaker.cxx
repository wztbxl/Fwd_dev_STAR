...
{ 
    ...
    // if ( muDstMaker ){
    //     auto muDst = muDstMaker->muDst();
    //     auto muEvent = muDst->event();

    //     for ( size_t i = 0; i < muEvent->triggerIdCollection().nominal().triggerIds().size(); i++ ){
	// 		LOG_INFO  << "trigger : " << muEvent->triggerIdCollection().nominal().triggerIds()[i] << endm;
	// 	}
    // }

    // if ( this->mEvent->triggerIdCollection() ) {
    //     for ( size_t i = 0; i < this->mEvent->triggerIdCollection()->nominal()->triggerIds().size(); i++ ){
    //         LOG_INFO  << "trigger : " << this->mEvent->triggerIdCollection()->nominal()->triggerIds()[i] << endm;
    //     }
    // }
...
StFttQAMaker::MakeRawHitQA(){
    ...
    for ( auto rawHit : mFttCollection->rawHits() ) {
        ...
        mFttData.stripcenter[mFttData.N]     = rawHit->strip_center();
...
void StFttQAMaker::MakeClusterQA(){
    ...
    for ( auto clu : mFttCollection->clusters() ) {
        ...
        mFttData.cMaxAdc_Pos[mFttData.cN]    = clu->SC_MaxStrip();
        mFttData.cMaxADC_LEdge[mFttData.cN]  = clu->MaxStripLEdge();
        mFttData.cMaxADC_REdge[mFttData.cN]  = clu->MaxStripREdge();
        // mH1d[ "U_MaxSc_Pos_diff" ]->Fill(clu->x()+clu->SC_MaxStrip());
        mH1d[ "U_MaxSc_Pos_diff" ]->Fill(clu->x()-clu->SC_MaxStrip());
...
void StFttQAMaker::MakePointQA(){
    ...
    LOG_INFO << "MakePointQA :: Event " << GetIventNumber() << " has " << mFttCollection->points().size() << " Points" << endm;
    ...
    for ( auto p : mFttCollection->points() ) {
        ...
        mFttData.pDiag1[mFttData.pN] = p->diag1();
        mFttData.pDiag2[mFttData.pN] = p->diag2();
        ...
        p->print();
        mH1d[ "XY_U_diff" ]->Fill( (p->x()+p->y())-(p->diag1()*sqrt(2)) );
        // if(p->diag2() > 1) mH1d[ "XY_U_diff" ]->Fill( (p->x()+p->y())-(p->diag2()*sqrt(2)) );
        for ( auto clu : mFttCollection->clusters() )
        {
            if( clu->plane() == p->plane() && clu->quadrant() == p->quadrant() && (clu->orientation() == 2 || clu->orientation() ==3 ))
            {
                double diff = (p->x()+p->y())- (clu->x()*sqrt(2));
                mH1d[ "XY_U_diff_loop_MaxSC" ]->Fill(diff);
                diff = (p->x()+p->y())- (clu->SC_MaxStrip()*sqrt(2));
                mH1d[ "XY_U_diff_loop_diag" ]->Fill(diff);
            }
        }
...
StFttQAMaker::BookHistograms()
{
    ...
    mH1d[ "XY_U_diff" ]     = new TH1F( "XY_U_diff", ";(x+y)-u (mm); Counts", 1000, -30, 30 );
    mH1d[ "XY_U_diff_loop_MaxSC" ]     = new TH1F( "XY_U_diff_loop_MaxSC", ";(x+y)-u (mm); Counts", 4000, -200, 200 );
    mH1d[ "XY_U_diff_loop_diag" ]     = new TH1F( "XY_U_diff_loop_diag", ";(x+y)-u (mm); Counts", 4000, -200, 200 );
    mH1d[ "U_MaxSc_Pos_diff" ]     = new TH1F( "U_MaxSc_Pos_diff", ";MaxADC_SC - ClusterPos (mm); Counts", 4000, -200, 200 );
...
StFttQAMaker::BookTree()
{
    ...
    mFttTree->Branch("stripCenter"    , mFttData.stripcenter    , "stripcenter[N]/F");
    ...
    mFttTree->Branch("cMaxAdc_Pos"    , mFttData.cMaxAdc_Pos    , "cMaxAdc_Pos[cN]/F");
    mFttTree->Branch("cMaxADC_LEdge"  , mFttData.cMaxADC_LEdge  , "cMaxADC_LEdge[cN]/s");
    mFttTree->Branch("cMaxADC_REdge"  , mFttData.cMaxADC_REdge  , "cMaxADC_REdge[cN]/F");
    ...
    mFttTree->Branch("pDiag1"   , mFttData.pDiag1   , "px[pN]/F");
    mFttTree->Branch("pDiag2"   , mFttData.pDiag2   , "py[pN]/F");
