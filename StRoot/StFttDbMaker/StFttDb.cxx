...
double StFttDb::pent_shift = 101.6;//mm (From simulator, may updated in the real geo)
double StFttDb::idealPlaneZLocations[] = { 280.90499, 303.70498, 326.60501, 349.40499 };
double StFttDb::HV_Strip_shift = 15.95;//mm
double StFttDb::Diag_Strip_shift = 19.42;//mm
...
bool StFttDb::loadStripCenterFromFile( std::string fn ){
    ...
    if ( idx1 == string::npos ) // load the Row4(Diagonal)
    {
        ...
        while (inf >> nStrip >> StripCenter)
        {
            ...
            scMap_Diag[n_strip] = pos_strip_center;
        }
    ...
    if ( idx2 == string::npos ) // load the Row1(H&V)
    {
        ...
        while (inf >> nStrip >> StripCenter)
        {
            ...
            scMap_XY[n_strip] = pos_strip_center;
        }
...
bool StFttDb::loadStripEdgeFromFile( std::string fn ){
    ...
    if ( idx1 != string::npos ) // load the Row4(Diagonal)
    {
        ...
        while (inf >> nStrip >> StripEdge_L >> StripEdge_R)
        {
            ...
            seMap_Diag_L[n_strip] = pos_strip_edge_L;
            seMap_Diag_R[n_strip] = pos_strip_edge_R;
        }
...
// a little bit difference with Daniel's version 
UChar_t StFttDb::getOrientation( int rob, int feb, int vmm, int row ) const {
    if ( mDebug ) {
        printf( "getOrientation( %d, %d, %d, %d )", rob, feb, vmm, row ); puts("");
    }

    //since feb start from 0, the even feb should be the front chamber and the odd feb should be the back chamber
    if ( rob % 2 == 0 ){ // even rob
        if ( feb % 2 == 0 ) { // odd
            // row 3 and 4 are always diagonal
            if ( 3 == row || 4 == row )
 ...
 *
 */
bool StFttDb::hardwareMap( int rob, int feb, int vmm, int ch, int &row, int &strip, UChar_t &orientation ) {
    uint16_t key = packKey( feb, vmm, ch );
    if ( mMap.count( key ) ){
...
bool StFttDb::hardwareMap( StFttRawHit * hit ) {
    ...
    if ( mMap.count( key ) ){
        ...
        Float_t strip_center = -1;
        Float_t strip_LEdge = -1;
        Float_t strip_REdge = -1;
        if (orientation == kFttHorizontal || orientation == kFttVertical)
            strip_center = scMap_XY[ strip ];
        if (orientation == kFttDiagonalH || orientation == kFttDiagonalV)
        {
            strip_center = scMap_Diag[ strip ];
            strip_LEdge = -1;
            strip_REdge = -1;
            strip_LEdge = seMap_Diag_L[ strip ];
            strip_REdge = seMap_Diag_R[ strip ];

        }
        // cout << " iPlane = " << (int)iPlane << " iQuad = " << (int)iQuad << " row = " << row << " strip = " << strip << " strip center = " << strip_center << " left edge = " << strip_LEdge << " right edge = " << strip_REdge << endl;
        hit->setMapping( iPlane, iQuad, row, strip, strip_center, orientation );
        hit->setStripEdge( (Float_t)strip_LEdge, (Float_t)strip_REdge );
        return true;
    }
    return false;
}
